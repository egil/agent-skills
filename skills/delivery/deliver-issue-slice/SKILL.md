...
Before delegating pending work, selecting a timeout, or recovering after handoff, resume, compaction, or a side task, apply [message-driven waiting](../delivery-runtime-protocol/references/message-driven-waiting.md). Restore its deadline and session-local limits, then calculate the timeout. Keep results inside this issue's ownership chain; preserve the selected role-reuse policy, exact-snapshot receipts, and required checks.
...
Before delegating or recovering implementation or testing phases, apply [same-issue worker continuity](../delivery-runtime-protocol/references/worker-continuity.md). Continue as the issue Implementor and resume its matching Tester for corrections, green finalization, and test-owned rebase conflicts. Keep independent Reviewer selection unchanged.
...
6. Reconcile the latest contract-defined phase checkpoint with the linked remote SHA and any pull-request head. Recover the referenced Tester and Reviewer receipts. If the checkpoint is missing, inconsistent, or covers another SHA, stop at the last proven phase; any nondurable gate whose clean result cannot be retrieved must be rerun as a bounded assignment on the recovered exact snapshot. Resume the matching Tester under worker continuity; retain fresh independent Reviewer requirements. Never infer a phase or pass from branch contents.
...
- the matching issue Tester owns approved test-code changes, including justified reconsideration of a reviewed test; replace it only through the worker-continuity branch;
...
- process findings in file order within each axis, one at a time. For each production finding, make and verify the production change; for each test finding, resume the matching Tester with that finding ID and exact current snapshot. Then update only `dispositions.md` with the decision, rationale, exact resolution commit when applicable, and verification evidence before taking the next finding;
- after any production or test change, run applicable verification, resume the matching Tester for green finalization when assertions changed, curate history, and commission fresh complete-change review in the new `HEAD` snapshot directory. A clean review still writes its exact-snapshot result receipt without changing Git state; and
