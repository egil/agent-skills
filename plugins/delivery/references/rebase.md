...
2. Resolve and stage production-owned conflicts. For test-owned conflicts, resume the matching Tester in `rebase-conflict` mode under same-issue worker continuity with that exact state and leave Git, index, and worktree untouched while it runs.
